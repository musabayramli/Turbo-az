...
function showElan() {
  document
    .querySelector(".nav-accordion")
    .classList.toggle("nav-accordion-active");
  if (!flag) {
    document.querySelector(
      ".nav-acc-ac"
    ).innerHTML = `Gizlət <i class="fa-solid fa-chevron-up"></i>`;
    flag = true;
  } else {
    document.querySelector(
      ".nav-acc-ac"
    ).innerHTML = `Daha çox filtr <i class="fas fa-chevron-down"></i>`;
    flag = false;
  }
}

marka.map(
  (item) => (markaDesk.innerHTML += `<option value="${item}">${item}</option>`)
);
city.map(
  (item) => (cityDesk.innerHTML += `<option value="${item}">${item}</option>`)
);
currency.map(
  (item) => (val.innerHTML += `<option value="${item}">${item}</option>`)
);
ban.map(
  (item) => (banDesk.innerHTML += `<option value="${item}">${item}</option>`)
);
...
function ModelSec() {
  ...
  let models = [
    ...new Set(
      cars
        .filter((item) => markaDesk.value == item.brand)
        .map((item) => item.model)
    ),
  ];
  for (let i = 0; i < models.length; i++)
    kod += `<option>${models[i]}</option>`;
  modelDesk.innerHTML = kod;
}

function VbtnColor(z) {
  for (let i = 0; i < vBtns.length; i++)
    vBtns[i].classList.remove("vezyet-btn-active");
